...
GradientDialog::GradientDialog(Function *func, Area area, double step_size, int n_iter, QWidget *parent) :
    QDialog(parent), \
    ...
    _start_point[0] = (_left_bound[0] + _right_bound[0]) / 2;
    _start_point[1] = (_left_bound[1] + _right_bound[1]) / 2;
    ui->setupUi(this);
...
void GradientDialog::mousePressEvent(QMouseEvent *event) {
    QPoint point = event->pos();
    _start_point[0]=ui->plot->xAxis->pixelToCoord(event->x());
    _start_point[1]=ui->plot->yAxis->pixelToCoord(event->y());
    _eps = ui->epsilon->value();
    _stop_criteria = ui->stop->currentIndex();
    Area *area = new Area(_left_bound, _right_bound);
    Criteria *criteria;
    switch(_stop_criteria){
        case 0: criteria = new CriteriaValue; break;
        case 1: criteria = new CriteriaArgument; break;
        case 2: criteria = new CriteriaGradient; break;
        default: criteria = new CriteriaSteps; break;
    }
    ...
    Optimization*opt=new Gradient(_func, area, criteria, _start_point);
